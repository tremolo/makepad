...
/// Map a raw Linux evdev keycode to a Makepad KeyCode.
///
/// This is layout-independent: evdev codes identify physical key positions on an AT-101
/// keyboard, regardless of the current keyboard layout (US, German, French, etc.).
/// The Wayland `wl_keyboard::Event::Key` delivers these codes directly; XKB adds 8 to
/// get its own "XKB keycode", so callers should pass the raw Wayland `key` value here,
/// not `key + 8`.
pub fn evdev_to_makepad_keycode(key: u32) -> crate::event::keyboard::KeyCode {
    use crate::event::keyboard::KeyCode;
    match key {
        1 => KeyCode::Escape,
        2 => KeyCode::Key1,
